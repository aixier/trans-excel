...
class SessionCleaner:
    ...
    async def _clean_expired_sessions(self) -> Dict[str, int]:
        ...
        # Get all sessions from session manager (pipeline architecture)
        ...
        for session_id, session in sessions.items():
            try:
                ...
                if created_at and created_at < cutoff_time:
                    ...
                    # Remove session (pipeline architecture)
                    pipeline_session_manager.delete_session(session_id)
                    cleaned_count += 1
    ...
    async def force_clean_session(self, session_id: str) -> bool:
        ...
        """
        try:
            # Remove from session manager (pipeline architecture)
            pipeline_session_manager.delete_session(session_id)
