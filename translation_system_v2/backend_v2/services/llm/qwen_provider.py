...
class QwenProvider(BaseLLMProvider):
    ...
    async def _call_api_with_retry(self, request: Dict[str, Any]) -> tuple:
        ...
        # 🔍 打印完整的API请求
        logger.info("="*100)
        logger.info("🚀 LLM API 请求")
        logger.info("="*100)
        logger.info(f"📡 URL: {self.base_url}/services/aigc/text-generation/generation")
        logger.info(f"🔑 Model: {request['model']}")
        logger.info(f"📋 完整请求体:")
        logger.info(json.dumps(request, ensure_ascii=False, indent=2))
        logger.info("="*100)
