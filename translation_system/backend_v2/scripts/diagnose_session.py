...
def diagnose_session(session_id: str):
    ...
    stage = session.session_status.stage
    print(f"\n📊 Session 阶段: {stage.value}")
    stage_map = {
        SessionStage.CREATED: "已创建（需要上传分析）",
        SessionStage.ANALYZED: "已分析（可以拆分任务）",
        SessionStage.SPLIT_COMPLETE: "拆分完成（可以执行翻译）",
        SessionStage.EXECUTING: "执行中",
        SessionStage.COMPLETED: "已完成",
        SessionStage.FAILED: "失败"
    }
    ...
    print(f"\n📁 Excel 数据:")
    if session.excel_df:
        sheets = session.excel_df.get_sheet_names()
