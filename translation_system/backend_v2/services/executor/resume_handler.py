...
class ResumeHandler:
    ...
    async def create_checkpoint(self, session_id: str) -> str:
        ...
        try:
            ...
            serialization_data = {
                ...
                'game_info': game_info.__dict__ if game_info else {},
                'analysis': analysis or {},
                'session_metadata': session_data.metadata if session_data else {}
            }
            ...
            session_manager.set_metadata(session_id, 'latest_checkpoint', str(checkpoint_path))
    ...
    async def restore_checkpoint(self, checkpoint_path: str) -> str:
        ...
        try:
            ...
            game_info_dict = data['game_info']
            analysis = data['analysis']
            session_metadata = data['session_metadata']

            original_session_id = checkpoint_info['session_id']

            # Create new session ID for restoration
            new_session_id = session_manager.create_session()

            # Restore Excel DataFrame
            session_manager.set_excel_df(new_session_id, excel_df)
            ...
            if task_records:
                ...
                session_manager.set_task_manager(new_session_id, task_manager)

            # Restore Game Info
            if game_info_dict:
                game_info = GameInfo()
                for key, value in game_info_dict.items():
                    setattr(game_info, key, value)
                session_manager.set_game_info(new_session_id, game_info)

            # Restore analysis and metadata
            if analysis:
                session_manager.set_analysis(new_session_id, analysis)

            for key, value in session_metadata.items():
                session_manager.set_metadata(new_session_id, key, value)

            # Mark as restored session
            session_manager.set_metadata(new_session_id, 'restored_from', original_session_id)
            session_manager.set_metadata(new_session_id, 'restored_at', datetime.now().isoformat())
            session_manager.set_metadata(new_session_id, 'checkpoint_path', checkpoint_path)
    ...
    async def find_latest_checkpoint(self, session_id: str) -> Optional[str]:
        ...
        try:
            ...
            latest_checkpoint = session_manager.get_metadata(session_id, 'latest_checkpoint')
